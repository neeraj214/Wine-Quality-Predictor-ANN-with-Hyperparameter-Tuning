...
def perform_random_search():
    ...
    print(f"Initializing Random Search with {X_train.shape[0]} samples...")
    ...
    def model_factory(neurons=128, layers=2, learning_rate=0.001, dropout=0.3):
        ...
        # Re-compile with sparse loss to work with integer labels in RandomizedSearchCV
        model.compile(
    ...
    param_dist = {
        'neurons': [64, 128, 256],
        'layers': [1, 2, 3],
        'learning_rate': uniform(0.0001, 0.0099), # range [0.0001, 0.01]
        'batch_size': [32, 64],
        'epochs': [50]
    }

    print("Starting RandomizedSearchCV (n_iter=10, cv=3, n_jobs=-1)...")
    ...
    search_result = random_search.fit(X_train, y_train_indices)
    ...
    with open('outputs/random_search_best_params.json', 'w') as f:
        json.dump(search_result.best_params_, f, indent=4)
    print("\nBest parameters saved to outputs/random_search_best_params.json")

    # Save the best model
    os.makedirs('models', exist_ok=True)
    best_model = search_result.best_estimator_.model
    best_model.save('models/random_search_best_model.h5')
    print("Best model saved to models/random_search_best_model.h5")
